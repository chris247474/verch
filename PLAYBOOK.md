# Verch Local Agent Playbook

This playbook is a practical guide to run Verch with a local model (Ollama), work safely, and get strong day-to-day results.
...
If everything is healthy, Verch starts directly.
...
Or let Verch recommend the best local model for your goal:
...
```powershell
Get-Content .\.verch-profile.json
```
...
- Keep `.verch-profile.json` local (already gitignored).
